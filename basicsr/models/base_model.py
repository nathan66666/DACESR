...
class BaseModel():
    ...
    #         net_g_ema = getattr(self, 'net_g_ema_{}'.format(str(i)))
    ...
    def model_ema(self, decay=0.999):
        ...
        for k in net_g_ema_params.keys():
            # print(f"EMA param shape: {net_g_ema_params[k].shape}, Current param shape: {net_g_params[k].shape}")
            net_g_ema_params[k].data.mul_(decay).add_(net_g_params[k].data, alpha=1 - decay)
            # with torch.no_grad():
            #     net_g_ema_params[k].mul_(decay).add_(net_g_params[k], alpha=1 - decay)


    def iqa_model_ema(self, decay=0.999):
    ...
    def model_interpolation(self, factor):
    ...
    def load_network_init(self, net, load_path, num_networks, strict=True, param_key='params'):
        """Load network.
    ...
    def load_network_init_alldynamic(self, net, load_path, num_networks, strict=True, param_key='params'):
        """Load network.
        ...
        load_net_repeated = {}
        for k, v in load_net.items():
            # print(v.shape,v.unsqueeze(0).shape)
            load_net_repeated[k] = v
            # if 'weight' in k:
            #     # if v.dim() == 3:
            #     #     load_net_repeated[k] = v.unsqueeze(0).repeat(num_networks, 1, 1, 1)
            #     # elif v.dim() == 4:
            #     #     load_net_repeated[k] = v.unsqueeze(0).repeat(num_networks, 1, 1, 1, 1)
            #     load_net_repeated[k] = v.repeat(num_networks, 1, 1, 1, 1)

            #     # load_net_repeated[k] = v.unsqueeze(0).repeat(num_networks, 1, 1, 1, 1)
            # elif 'bias' in k:
            #     # load_net_repeated[k] = v.unsqueeze(0).repeat(num_networks, 1)
            #     load_net_repeated[k] = v.repeat(num_networks, 1)
            # else:
            #     load_net_repeated[k] = v
        # # # remove unnecessary 'module.'
        # for k, v in deepcopy(load_net).items():
