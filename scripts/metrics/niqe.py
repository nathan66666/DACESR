...
def calculate_niqe(img2, crop_border, input_order='HWC', convert_to='y'):
    """Calculate NIQE (Natural Image Quality Evaluator) metric.
    ...
    # we use the official params estimated from the pristine dataset.
    niqe_pris_params = np.load('/lxy/DASR/basicsr/metrics/niqe_pris_params.npz')
...
if __name__ == '__main__':
    ...
    img_folder = '/lxy/DASR/results/test_DVMSR/visualization/RealLR200'
    img_files = [f for f in os.listdir(img_folder) if f.endswith(('.png', '.jpg', '.jpeg'))]
    all_score = 0
    count = 0
    for img_file in tqdm(img_files):

        img_restored = cv2.imread( os.path.join(img_folder,img_file), cv2.IMREAD_UNCHANGED).astype(
                                np.float32)
        
        score = calculate_niqe(img_restored, crop_border=4, input_order='HWC', convert_to='y')
        count += 1
        all_score += score
        print(score)
    avg_score = all_score / count
    print(avg_score)